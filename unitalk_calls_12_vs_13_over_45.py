...
def build_report():
    ...
    date_to = min(now, thirteen)

    managers = get_managers()
    calls = get_calls(date_from, date_to)

    counts_12 = {line: 0 for line in managers}
    counts_13 = {line: 0 for line in managers}
    ...
    for call in calls:
        ...
        # Станом на 13:00:
        # усі дзвінки >=45 сек від початку дня до 13:00.
        if call_time < thirteen:
            counts_13[manager_line] += 1

        # Станом на 12:00:
        # усі дзвінки >=45 сек від початку дня до 12:00.
        if call_time < twelve:
            counts_12[manager_line] += 1
    ...
    # Сортуємо за показником на 13:00,
    # далі за показником на 12:00.
    rows.sort(
        key=lambda row: (
            -row[2],
            -row[1],
